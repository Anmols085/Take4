...
class Message(object):
    ...
    def apply_shift(self, shift):
        ...
        s = ''
        m = Message()
        newDict = m.build_shift_dict(shift)
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        maximum = 0
        s = 0
        p = self.message_text.split()
        realMessage = ''
        for i in range(0, 27):
            count = 0
            for string in p:
                m = Message(string)
                newMessage = m.apply_shift(i)
                if(is_word(is_word("words.txt"), newMessage)):
                    count+=1
            if(count>maximum):
                maximum = count
                s = i
...
def decrypt_story():
    newC = CiphertextMessage(get_story_string())
    return newC.decrypt_message()
